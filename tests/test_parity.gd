...
## Run: godot --headless --path addons/godot-mcp --script res://tests/test_parity.gd
...
func _run() -> void:
	...
	# --- project settings ---
	_check(cmds.dispatch("get_project_setting", { "setting": "application/config/name" }).value == "godot_mcp_tests", "get_project_setting name")
